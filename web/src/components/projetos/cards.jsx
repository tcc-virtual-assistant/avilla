...
function ProjectCards() {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:8003/projects')
    ...
    const url = `http://localhost:8003/projects/media/${data.image_path}`
                        ...
                        {data.map((project, index) => (
                            <div key={project.project_id} className="shadow-md flex">
                                <div className=" card card-side bg-base-200 shadow-xl rounded" >
                                    <figure><img src={`http://127.0.0.1:8003/projects/media/${project.image_path}`} className="w-80  h-full" alt="Movie" /></figure>
